...
AutoSequelize.prototype.build = function(callback) {
  ...
  function processTables(__tables) {
    ...
    function mapTables(err) {
      ...
      async.each(tables, mapIndexKeys, ()=>{
  ...
  function mapIndexKeys(table, fn) {
    self.queryInterface.showIndex(table, {
      raw: true
    }).then((res) => {
      if(res.length > 1) {
        _.each(res, assignIndexDetails)
        fn();
      } else {
        fn();
      }
    })
    function assignIndexDetails(ref) {
      self.indexKeys[table] = self.indexKeys[table] || [];
      if(!ref.primary) {
        delete ref.tableName;
        delete ref.primary;
        if(ref.type === "BTREE") {
          delete ref.type;
        }
  ...
  function generateText(err) {
    ...
    async.each(_.keys(self.tables), function(table, _callback){
      ...
      if(!self.options.typescript){
        ...
        if (self.options.egg) {
          text[table] = tsHelper.model.getModelEggFileStart(self.options.indentation, spaces, tableName);
        } else {
      ...
      // generate Indexs
      if(self.indexKeys[table]) {
        text[table] += spaces + spaces  + "indexes: " + JSON.stringify(self.indexKeys[table]) +",\n";
      }
      ...
      // replace json key
      text[table] = text[table].replace(/(\"|\')([\w_]+)\1\:/g,'$2:').replace(/\"/g,'\'');
      _callback(null);
    }, function(){
